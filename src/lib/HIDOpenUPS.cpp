...
ATXMSG g_OpenUPS_memMessages[MAX_MESSAGE_CNT] = 
{
	...
	{70, 1, _T("CELLS")								, true, 1, 1		, _T("Number of configured cells to charge/balance\r\nDefault is 1."), _T("[pcs]") },
...
/***/{ 6, 2, _T("UPS_VIN_MIN_STOP")					, true,11, 0		, _T("If input voltage is below this threshold the UPS will try to run on battery.\r\nDefault is 6V."), _T("[V]") },
	{ 8, 2, _T("UPS_VCELL_MIN_START")				, true,10, 2		, _T("If Vin is not present and all the battery cells are above this threshold the UPS can start and runs on battery\r\nDefault is 11.7V."), _T("[V]") },
	{10, 2, _T("UPS_VCELL_MIN_STOP")				, true,10, 2		, _T("If VIN is not present and one of the battery cells is less than this threshold during the time period specified by  UPS_VBAT_UVP_OFF_TOUT parameter the UPS will initiate shut down procedure.\r\nDefault is 11.4V."), _T("[V]") },
	...
	{101,2, _T("UPS_VBAT_SLEEP_TOUT")				, true, 3, 1		, _T("Deep sleep timeout (Firmware V1.4+).  If system is running on battery and this period has elapsed with no input power applied then the UPS will initiate shut down procedure.\r\n\"Never\" is allowed as value (to disable this feature)"), _T("[HH:MM:SS]") },
	
//	{16, 2, _T("UPS_VBAT_PRESENT")					, true,10, 2		, _T("If VBAT< VBAT_PRESENT the battery is considered not present(used for powering the device only from USB for configuration,so the device won't enter deep sleep etc."), _T("[V]") },
...
//	{24, 4, _T("CAPACITY")							, true,16, 4		, _T("Battery capacity"), _T("[mAh]") }, //old style with constant
// moved up	{28, 4, _T("CAPACITY")							, true,16, 1		, _T("Battery Capacity.\r\nDefault 3400mAh"), _T("[mAh]") },
 
//	{32, 2, _T("CHG_COND_VBAT_MIN")					, true,10, 2		, _T("Charge qualification is enabled if Battery voltage is less than this value"), _T("[V]") },
	{34, 2, _T("CHG_ICOND")							, true,10, 3		, _T("Conditioning/Precharge current. Charge current is limited to this value until cell voltage exceeds CHG_VCOND value and for at least CHG_TCOND time.\r\nDefault is 100mA."), _T("[mA]") },
	{36, 2, _T("CHG_VCOND")							, true,10, 2		, _T("Conditioning/Precharge voltage. Charge current is limited to CHG_ICOND until cell voltage exceeds this value and for at least CHG_TCOND time.\r\nDefault is 11.2V."), _T("[V]") },
	{38, 2, _T("CHG_TCOND")							, true, 1, 1		, _T("Conditioning/Precharge time. Charge current is limited to  CHG_ICOND value until cell voltage exceeds CHG_VCOND value and it is applied during this time.\r\nDefault is 30 sec."), _T("[s]") },
	{40, 2, _T("CHG_IBULK")							, true,10, 3		, _T("Fast charge current limit (constant current mode)\r\nDefault is 1750mA."), _T("[mA]") },
	{42, 2, _T("CHG_BULK_STOP_VOLTAGE")				, true,10, 2		, _T("Maximum allowed bulk charge voltage/cell during constant current/constant voltage charging\r\nDefault is 14.1V"), _T("[V/cell]") },
	{71, 1, _T("CHG_HYSTERESIS")					, true,10, 2		, _T("An overvoltage value (CHG_BULK_STOP_VOLTAGE +CHG_HYSTERESIS ) that it is allowed when charging.If one of the cells exceeds this value charging is imediately stopped.\r\nDefault is 100mV/cell."), _T("[V/cell]") },
//	{44, 2, _T("CHG_TBULK")							, true, 1, 1		, _T("Terminate bulk charge if charging for more than this period"), _T("[s]") },
	...
	{96, 2, _T("CHG_GLOBAL_TOUT")					, true, 1, 0.5		, _T("Global charge timeout (Firmware v1.3+)"), _T("[min]") },
	...
	{48, 2, _T("CHG_START_VOLTAGE")					, true,10, 2		, _T("If cell voltage is below this value charging can be started. For SLA battery this also sets the floating charge voltage.\r\nDefault is 13.5V/cell"), _T("[V/cell]") },
	...
	{51, 2, _T("CHG_TEMP_PCB")						, true,15, 1		, _T("Charge current is limited by CHG_ILIMIT_TEMP_PCB amount every 30s,if board temperature exceeds this value. Can limit the charge current till CHG_IBULK/4 value.\r\nDefault is 55�C."), _T("[�C]") },
/***/{103,2, _T("CHG_TEMP_PCB_LOW")					, true,15, 1		, _T("Charge current is limited by CHG_ILIMIT_TEMP_PCB amount every 30s,if board temperature is lower than this value for SLA, turns off charging for LiIon and LiFePo4. Can limit the charge current till CHG_IBULK/4 value(Firmware V1.6+)\r\nDefault is 5�C."), _T("[�C]") },
	{53, 2, _T("CHG_ILIMIT_TEMP_PCB")				, true,10, 3		, _T("Limit charge current with this amount if board temperature exceeds CHG_TEMP_PCB.\r\nDefault is 50mA."), _T("[mA]") },
//not implemented yet	{55, 1, _T("CHG_TEMP_CELL_LIMIT")			    , true, 1, 1		, _T("Charge current is limited by CHG_ILIMIT_TEMP_CELL amount if cell temperature exceeds this value"), _T("[�C]") },
//not implemented yet	{56, 2, _T("CHG_ILIMIT_TEMP_CELL")				, true,10, 3		, _T("Limit charge current with this amount if cell  temperature exceeds CHG_TEMP_CELL_LIMIT"), _T("[mA]") },
//not implemented yet	{58, 1, _T("CHG_TEMP_CELL_CUTOFF")			    , true, 1, 1		, _T("Charge current is terminated if if cell temperature exceeds this value"), _T("[�C]") },
/***/{59, 1, _T("CHG_FREQUENCY")					, true,12, 1		, _T("The working frequency of the charger buck-boost converter\r\nDefault is 333kHz."), _T("[kHz]") },

	{60, 2, _T("BAL_VCELL_MIN")						, true,10, 2		, _T("Balancing is allowed  if cell voltages are above this value.\r\nDefault is 3V"), _T("[V]") },
	{62, 1, _T("BAL_VCELL_DIFF_START")				, true,10, 2		, _T("If the voltage difference between cells exceeds this value start balancing the cells.\r\nDefault is 70mV."), _T("[V]") },
	{63, 1, _T("BAL_VCELL_DIFF_STOP")				, true,10, 2		, _T("If the voltage difference between cells is less than this value stop balancing the cells.\r\nDefault is 40mV."), _T("[V]") },
	
	{64, 2, _T("OUT_VOLTAGE")						, true,7 , 1		, _T("The output voltage.\r\nDefault is 12V."), _T("[V]") },
/***/{66, 1, _T("OUT_FREQUENCY")					, true,13, 1		, _T("The working frequency of the output buck-boost converter.\r\nDefault 300kHz"), _T("[kHz]") },
...
//moved up	{70, 1, _T("CELLS")								, true, 1, 1		, _T("Number of configured cells to charge/balance\r\nDefault is 4."), _T("[pcs]") },
//moved up	{71, 1, _T("CHG_HYSTERESIS")					, true,10, 2		, _T("An overvoltage value (CHG_BULK_STOP_VOLTAGE +CHG_HYSTERESIS ) that it is allowed when charging.If one of the cells exceeds this value charging is imediately stopped.\r\nDefault is 200mV/cell."), _T("[V]") },
	{72, 4, _T("POUT_LO")							, true,16, 0.000001	, _T("Output power low threshold for motherboard alive sensing.If output power is lower than this threshold shut down impulse is NOT sent to the motherboard.\r\nDefault is 2W."), _T("[W]") },
	{76, 4, _T("POUT_HI")							, true,16, 0.000001	, _T("Output power high threshold for motherboard alive sensing. If output power is higher than this threshold shut down impulse is sent to the motherboard.\r\nDefault is 6W."), _T("[W]") },
...
//moved up	{80, 2, _T("MOB_ONOFF_TOUT")					, true, 1, 10		, _T("The powerswitch on the motherboard(if connected) will be shorted  this period to turn the Motherboard ON or OFF\r\nDefault is 500ms"), _T("[mS]") },
	...
	{82, 2, _T("OCV_SOC0")							, true,10, 2		, _T("Open Circuit Voltage State Of Charge detection for initial 0% fuel gauge estimation.\r\nDefault is 11.8V"), _T("[V]") },
	{84, 2, _T("OCV_SOC10")							, true,10, 2		, _T("Open Circuit Voltage State Of Charge detection for initial 10% fuel gauge estimation.\r\nDefault is 11.9V"), _T("[V]") },
	{86, 2, _T("OCV_SOC25")							, true,10, 2		, _T("Open Circuit Voltage State Of Charge detection for initial 25% fuel gauge estimation.\r\nDefault is 12V"), _T("[V]") },
	{88, 2, _T("OCV_SOC50")							, true,10, 2		, _T("Open Circuit Voltage State Of Charge detection for initial 50% fuel gauge estimation.\r\nDefault is 12.3V"), _T("[V]") },
	{90, 2, _T("OCV_SOC75")							, true,10, 2		, _T("Open Circuit Voltage State Of Charge detection for initial 75% fuel gauge estimation.\r\nDefault is 12.6V"), _T("[V]") },
	{92, 2, _T("OCV_SOC100")						, true,10, 2		, _T("Open Circuit Voltage State Of Charge detection for initial 100% fuel gauge estimation.\r\nDefault is 12.8V"), _T("[V]") },
...
//moved up	{98, 2, _T("CHG_IFLOAT")						, true,10, 3		, _T("Charge current limit in float charge mode (Firmware v1.3+)"), _T("[mA]") },
...
//moved up	{103,2, _T("CHG_TEMP_PCB_LOW")					, true,15, 1		, _T("Charge current is limited by CHG_ILIMIT_TEMP_PCB amount every 30s,if board temperature is lower than this value for SLA, turns off charging for LiIon and LiFePo4. Can limit the charge current till CHG_IBULK/4 value.\r\nDefaults is 0�C."), _T("[�C]") },
...
void HIDOpenUPS::WriteConfigurationMemory()
{
	...
	while (m_ulSettingsAddr < SETTINGS_ADDR_END)
	{
		sendMessageWithBuffer(OPENUPS_MEM_WRITE_OUT, 16, m_chPackages+(m_ulSettingsAddr-SETTINGS_ADDR_START), 4, m_ulSettingsAddr & 0xFF, (m_ulSettingsAddr >> 8) & 0xFF, 0x00, 0x10);
